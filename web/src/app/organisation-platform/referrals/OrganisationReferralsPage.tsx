...
function TierBadgeIcon({ tier }: { tier: OrganizationPartnerTier }) {
...
function TierCard({ tier }: { tier: OrganizationPartnerTier }) {
...
function buildOrganizationPartnerTiers(referrals: OrganizationReferrals | null): OrganizationPartnerTier[] {
  if (referrals?.tiers?.length) {
    return referrals.tiers.map((tier) => ({
      ...
      rewards: [
        { label: "Patient signup", value: formatOrganizationMoney(tier.rewards.patientAmountCents, tier.currency || referrals.metrics.currency) },
        { label: "Professional signup", value: formatOrganizationMoney(tier.rewards.professionalAmountCents, tier.currency || referrals.metrics.currency) },
        { label: "Organization onboarded", value: formatOrganizationMoney(tier.rewards.organizationAmountCents, tier.currency || referrals.metrics.currency) },
  ...
  return [
    {
      ...
      description:
        "Share your referral link with organizations, professionals, and patients. Rewards are tracked automatically after successful signups.",
      statusLabel: "Active",
      progress: 1,
      progressLabel: "Active",
      cardTone: "blue",
      rewards: [
        { label: "Patient signup", value: "Tracked" },
        { label: "Professional signup", value: "Tracked" },
        { label: "Organization onboarded", value: "Tracked" },
        { label: "Unlock criteria", value: "Verified account" },
      ],
    },
    {
    ...
    },
    {
      ...
      description: "For high-performing organizations with broad verified referral impact.",
...
export function OrganisationReferralsPage() {
  ...
  const partnerLevel =
    (referrals?.metrics.totalReferrals ?? 0) >= 100
      ? "Health Ambassador"
      : (referrals?.metrics.totalReferrals ?? 0) >= 20
        ? "Community Advocate"
        : "Referral Partner";
  const partnerTiers = useMemo(() => buildOrganizationPartnerTiers(referrals), [referrals]);
