...
export function OrganisationProfessionalsPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<"all" | ProfessionalStatus>(
    "all",
  );
        ...
        setSummaryCards([
          {
            ...
            value: String(nextProfessionals.length),
          },
          {
            title: "on shift now",
            value: String(
              nextProfessionals.filter((item) => item.status === "On shift")
                .length,
            ),
          },
          {
            ...
            value: String(
              nextProfessionals.filter((item) => item.status === "Available")
                .length,
            ),
          },
          {
            title: "Unavailable",
            value: String(
              nextProfessionals.filter((item) => item.status === "On leave")
                .length,
            ),
          },
        ]);
  ...
  const dateOptions = useMemo(
    () => [
      "all",
      ...Array.from(new Set(professionals.map((item) => item.date))),
    ],
    [professionals],
  );
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              ...
              >
                <select
                  ...
                  onChange={(event) =>
                    setStatusFilter(
                      event.target.value as "all" | ProfessionalStatus,
                    )
                  }
                  className="h-11 w-full cursor-pointer appearance-none rounded-[10px] border border-[#94A3B8] bg-white px-4 pr-12 text-[16px] tracking-[-0.05em] text-[#334155] outline-none transition duration-200 ease-out hover:border-[#1565C0] hover:bg-[#EFF6FF] focus:border-[#1565C0]"
                >
              ...
              <table className="w-full table-fixed border-separate border-spacing-0 text-left">
                ...
                <thead>
                  <tr className="bg-[#F8FAFC] text-[14px] text-[#94A3B8] shadow-[0_0_8px_rgba(21,101,192,0.1)]">
                    <th className="rounded-l-[6px] px-5 py-3 font-normal">
                      Professional
                    </th>
                    ...
                    <th className="rounded-r-[6px] px-5 py-3 font-normal">
                      Action
                    </th>
                  </tr>
                </thead>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[14px] text-[#1565C0]">
                          {item.role}
                        </p>
                      </div>
                    </div>
