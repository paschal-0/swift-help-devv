...
import { createPatientSymptomCheck, type PatientMedicalRecordsRecommendation } from "@/services/patientApi";

type SymptomAssessmentDraft = {
...
export function PatientSymptomCheckerRecommendationPage() {
  ...
  useEffect(() => {
    ...
    const rawDraft = window.sessionStorage.getItem("patientSymptomAssessmentDraft");
    if (!rawDraft) {
      toast.error("Complete an assessment before viewing a recommendation.");
      router.replace("/patient-platform/symptom-checker/assessment");
      return;
    }

    const draft = JSON.parse(rawDraft) as SymptomAssessmentDraft;
    const primarySymptom = draft.primarySymptoms?.join(", ") || "Reported symptoms";

    createPatientSymptomCheck({
      title: "Symptom assessment",
      symptoms: {
        primarySymptom,
        duration: draft.duration ?? "Not recorded",
        severity: draft.severity ?? "Not recorded",
        associatedSymptoms: draft.associatedSymptoms ?? "None recorded",
      },
      answers: {
        primarySymptoms: draft.primarySymptoms ?? [],
      ...
      .then((check) => {
        setRecommendation(check.recommendation as PatientMedicalRecordsRecommendation);
        window.sessionStorage.removeItem("patientSymptomAssessmentDraft");
      })
        ...
        <div className="mt-4 space-y-3 sm:mt-[20px] sm:space-y-4">
          <div className="rounded-[12px] bg-[#F8FAFC] px-3 pb-4 pt-3 transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_10px_24px_rgba(15,23,42,0.08)] sm:px-5 sm:pb-[18px] sm:pt-[17px]">
            <div className="inline-flex min-h-[30px] max-w-full items-center justify-center rounded-[24px] bg-[#F8FAFC] px-3 py-1 sm:h-[33px] sm:px-[10px]">
              <span className="text-[14px] font-normal leading-[18px] tracking-[-0.05em] text-[#1565C0] sm:text-[18px] sm:leading-[23px]">
                {recommendation?.headline ?? "Generating your recommendation"}
              </span>
            </div>
            <p className="mt-3 text-[14px] font-normal leading-[18px] tracking-[-0.05em] text-[#94A3B8] sm:mt-[15px] sm:text-[18px] sm:leading-[23px]">
              {recommendation?.description ?? "Your submitted symptom details are being reviewed."}
            </p>
          </div>
          ...
          <div className="rounded-[12px] bg-[#F8FAFC] px-3 pb-4 pt-3 transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_10px_24px_rgba(15,23,42,0.08)] sm:px-[22px] sm:pb-4 sm:pt-[14px]">
            ...
            <p className="mt-3 max-w-[595px] text-[14px] font-normal leading-[18px] tracking-[-0.05em] text-[#94A3B8] sm:mt-[14px] sm:text-[18px] sm:leading-[23px]">
              {recommendation?.whyRecommended ?? "Guidance will be based on the information you submitted."}
            </p>
          </div>
          ...
          <div className="rounded-[12px] bg-[#F8FAFC] px-3 pb-4 pt-3 transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_10px_24px_rgba(15,23,42,0.08)] sm:px-[18px] sm:pb-[13px] sm:pt-[15px]">
            ...
            <div className="mt-3 space-y-1.5 text-[14px] leading-[18px] tracking-[-0.05em] sm:text-[18px] sm:leading-[23px]">
              <p><span className="text-[#94A3B8]">-  Primary symptom:</span> <span className="font-medium text-[#334155]">{summary?.primarySymptom ?? "-"}</span></p>
              <p><span className="text-[#94A3B8]">-  Duration:</span> <span className="font-medium text-[#334155]">{summary?.duration ?? "-"}</span></p>
              <p><span className="text-[#94A3B8]">-  Severity:</span> <span className="font-medium text-[#334155]">{summary?.severity ?? "-"}</span></p>
              <p><span className="text-[#94A3B8]">-  Associated symptoms:</span> <span className="font-medium text-[#334155]">{summary?.associatedSymptoms ?? "-"}</span></p>
            </div>
          </div>
